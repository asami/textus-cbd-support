...
- [x] `P4-12` Snapshot retention has explicit source-count, observation-count, and memory/work bounds while preserving attributable last-known-good evidence.
- [x] `P4-13` Authentication, transport, parse, and compatibility failures have tested refresh-state transitions and never disguise stale evidence as current.
...
- [x] `P4-22` Live composed retrieval preserves CBD catalog/local evidence and SIE semantic evidence as separate source-owned records with bounded failures and no hidden winner.

...
final class CarReviewArtifactBundleSpec extends AnyWordSpec with Matchers with GivenWhenThen {
  ...
  private lazy val _response = {
    val attestation = CarReviewAttestationCodec.fromReport(_report).flatMap(CarReviewAttestationCodec.encode).fold(error => fail(error.message), identity)
    Printer.noSpaces.print(Json.obj(
      ...
      "report" -> parse(CarReviewReportCodec.encode(_report).fold(error => fail(error.message), identity)).fold(error => fail(error.message), identity),
      "attestation" -> parse(attestation).fold(error => fail(error.message), identity),
      "gateResult" -> Json.fromString(_report.gate.result.value)
    ))
  }
}

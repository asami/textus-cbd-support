...
import org.simplemodeling.textus.cbdsupport.runtime.{CarReviewDiagnosisAdmission, CarReviewDiagnosisTerminalState, CarReviewExecutionPlan, CarReviewRetentionPolicy}
import org.simplemodeling.textus.cbdsupport.entity.{ReviewDiagnosis as ReviewDiagnosisEntity, ReviewRunSnapshot as ReviewRunSnapshotEntity, ReviewReportSnapshot as ReviewReportSnapshotEntity, ReviewAttestationSnapshot as ReviewAttestationSnapshotEntity, ReviewRetentionEvent as ReviewRetentionEventEntity}
...
final class ComponentFactory extends CbdSupportComponent.Factory {
  ...
  private[cbdsupport] def _review_dashboard(
    ...
    new ReviewDiagnosisHistoryProgram(core).dashboard(reportId, roles)
  ...
  private[cbdsupport] def _review_diagnosis(
    ...
    new ReviewDiagnosisHistoryProgram(core).diagnosis(reportId, kind, itemId, roles)
  ...
  /**
   * Review diagnosis changes are ordinary SimpleEntity updates.  The Entity
   * layer owns the revision transition; this boundary supplies only the
   * domain fields and leaves inherited SimpleEntity attributes unchanged.
   */
  private def _diagnosis_update(
  ...
  private def _json_document(document: String): EntityReviewSubmissionDocument =
  ...
  private final class ReviewDiagnosisHistoryProgram(
    val core: ActionCall.Core
  ) extends ActionCall.Core.Holder with ActionCallEntityStorePart {
    ...
    def loadReport(
      ...
      for {
        ...
        report <- exec_from(_decode_exact(reportId, snapshots.data))
      } yield report
    ...
    ): Consequence[org.simplemodeling.textus.cbdsupport.runtime.CarReviewReport] =
      snapshots match {
